...
public class GroupedQuery2<Key, T1, T2> extends QueryBase {
     ...
     */
    public <R extends Result> LQuery<? extends R> select(@Expr(Expr.BodyType.Expr) Func1<Group2<Key, T1, T2>, R> expr) {
        throw new NotCompiledException();
    }
     ...
     * <b>注意：此函数的ExprTree[func类型]版本为真正被调用的函数
     */
    public boolean any(@Expr(Expr.BodyType.Expr) Func2<T1, T2, Boolean> func) {
        throw new NotCompiledException();
    }

    public boolean any(ExprTree<Func2<T1, T2, Boolean>> expr) {
     ...
     */
    public <R> List<Long> count(@Expr(Expr.BodyType.Expr) Func2<T1, T2, R> func) {
        throw new NotCompiledException();
    }
     ...
     */
    public <R extends Number> List<R> sum(@Expr(Expr.BodyType.Expr) Func2<T1, T2, R> func) {
        throw new NotCompiledException();
    }
     ...
     * @param func 返回需要统计的字段的lambda表达式(强制要求参数为<b>lambda表达式</b>，不可以是<span style='color:red;'>方法引用</span>以及<span style='color:red))
     */
    public <R extends Number> List<BigDecimal> avg(@Expr(Expr.BodyType.Expr) Func2<T1, T2, R> func) {
        throw new NotCompiledException();
    }
     ...
     * @param func 返回需要统计的字段的lambda表达式(强制要求参数为<b>lambda表达式</b>，不可以是<span style='color:red;'>方法引用</span>以及<span style='color:red)))
     */
    public <R extends Number> List<R> max(@Expr(Expr.BodyType.Expr) Func2<T1, T2, R> func) {
        throw new NotCompiledException();
    }
     ...
     * @param func 返回需要统计的字段的lambda表达式(强制要求参数为<b>lambda表达式</b>，不可以是<span style='color:red;'>方法引用</span>以及<span style='color:red)))
     */
    public <R extends Number> List<R> min(@Expr(Expr.BodyType.Expr) Func2<T1, T2, R> func) {
        throw new NotCompiledException();
    }
