...
public class OllamaDialect extends AbstractDialect {
    ...
    public boolean resolveResponseJson(ChatConfig config, ChatResponseDefault resp, String json) {
        ...
        if (oResp.contains("error")) {
            ...
            resp.created_at = oResp.get("created_at").getString();
            resp.done = oResp.get("done").getBoolean();
            resp.message = ChatMessage.of(config, oResp.get("message"));

            if (resp.done) {
                resp.done_reason = oResp.get("done_reason").getString();
