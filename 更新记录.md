#### 1.5.67
* 1. Bean 增加 index
* 2. Component 增加 index

#### 1.5.66
* 1.插件 water-solon-plugin 升级 water 2.3.2

#### 1.5.65
* 1. 缓存注解的 tags 值，支持返回数据做为模板参数
* 2. 国际化，支持 key 级别的 默认配置（之前基于文件）
* 3. 增加 solon.extend.hotdev 插件
* 4. 增加 国际化由过滤器自动为上下文解析地区

#### 1.5.63
1. 添加 solon.extend.graalvm 插件，用于适配 graalvm native image 模式
2. 添加 detector-solon-plugin 插件，用于为健康检测，增加一批预设的探测器（可选：cpu,disk,jvm,memory,os,qps）

#### 1.5.62
* 1.发布 sqltoy-solon-plugin 插件
* 2.优化 序列化插件关于 JsonActionExecutor 对数组数据的泛型处理

#### 1.5.61
* 1.调整 water-solon-plugin 内部的白名单机制

#### 1.5.60
* 1.内核 loadEnv 将同步到 System.setProperty 和 Solon.cfg().setProperty
* 2.增加新环境变量：solon.start.ping

#### 1.5.58
* 1.插件 solon.data 增加 CacheServiceProxy 类
* 2.内核 ResourceScaner 增加 resource 类型的资源扫描
* 3.增加 cloudevent-plus-solon-plugin 插件
* 4.增加 sqltoy-solon-plugin 插件

#### 1.5.57
* 1.插件 water-solon-plugin 增加基于服务名的消息订阅

#### 1.5.56
* 1.插件 water-solon-plugin 升级 water 2.3.0
* 2.简化接口 Utils::getResourceAsString(name)
* 3.插件 weed3-solon-puglin 升级 weed 3.4.8

#### 1.5.55
* 1.插件 solon.boot.smarthttp 升级 smart-http 为 1.1.9
* 2.插件 weed3-solon-puglin 升级 weed 3.4.7
* 3.修复 solon.extend.staticfiles 增加本地绝对位置时无效的问题
* 4.增加 app.before(index, handler) 接口
* 5.增加 app.cfg().isAloneMode() 接口（独立运行模式）

#### 1.5.54
* 1.插件 solon coud 事件总线，增加支持本地同主题多订阅模式（以支持同服务内，领域隔离的需求）
* 2.插件 solon.view.beetl，升级 beetl 到 3.8

#### 1.5.53
* 1.重新调整内核的异常处理链，进行让 filter 可以统一获取异常处理
* 2.设整 CrossHandler 接口，并增加 exposedHeaders(..)
* 3.插件 sa-token-solon-plugin，升级 sa-token 到 1.27.0
* 4.插件 beetlsql-solon-plugin，升级 beetlsql 到 3.11.0-RELEASE
* 5.增加 enableErrorAutoprint ，用于控制事件总线里的异常是否自动打印
* 6.调整 solon.logging 没有 solon 实例也可以使用

#### 1.5.50
* 1.添加 DownloadedFile 类，用于下载文件输出时用
* 2.将不确定的插件移到_hatch下

#### 1.5.49
* 1.插件 mybatis-solon-plugin 增加 mappers 单行配置支持

#### 1.5.48
* 1.插件 mybatis-solon-plugin 增加 SqlSessionFactoryBuilder 定制支持
