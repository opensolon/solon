#### 1.4.14
* 1.Mapping 的信号类型，由 HTTP 改为 ALL
* 2.所有模板增加 auth 标签支持
* 3.添加 AuthProcessorBase ，提供权限数组的配置方式
* 4.如果有主体处理，则设为setHandled(true)

#### 1.4.13
* 1.socketd 协议 headers 最长由 1k 增加为 4k
* 2.增加简化的配置文件名支持，例：app.yml, app-env.yml
* 3.opentracing-solon-plugin 增加 socketd 支持
* 4.nami debug 日志改由 slf4j 控制
* 5.socket debug 日志改由 slf4j 控制
...
* 2.取消 NamiAttachment，改由 NamiContext 接替  
* 3.添加 CloudJobHandler 接口（直接使用Handler，让人想不起来）
* 4.添加 Props::getByParse 接口
* 5.标注 Utils::throwableWrap 函数为弃用，并调整内部异常包装处理
...
* 7.调整 优先使用 system prop，并盖掉 solon prop；之后同时更新 system prop 和 solon prop
* 8.调整 solon auth 设计
