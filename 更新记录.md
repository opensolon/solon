...
|      |      |      |
| solon.cache.jedis     | solon.data.cache.jedis     | 增加与 solon.data 关联性     |
| solon.cache.spymemcached     | solon.data.cache.spymemcached     | 增加与 solon.data 关联性     |  
...
| solon.extend.validation.*     | solon.validation.*     | 地位升级     | 
| solon.extend.data.*     | solon.data.*     | 地位升级     |
| solon.cache.*     | solon.data.cache.*     | 增加与 solon.data 关联性     |
| solon.core.tran.*     | solon.data.tran.*     | 转到 solon.data 统一维护     |
| solon.core.cache.*     | solon.data.cache.*     | 转到 solon.data 统一维护     |
  

* 3.改动 ValidatorManager，由单例模式改为静态模式（更简洁些）
* 4.移动 org.noear.solon.core.cache.CacheService 到 org.noear.solon.data.cache.CacheService
* 5.移动 org.noear.solon.core.tran.TranExecutor  到 org.noear.solon.data.tran.TranExecutor
* 6.移动 org.noear.solon.core.tran.TranUtils     到 org.noear.solon.data.tran.TranUtils
* 7.更名 NoRepeatLock 更名为 NoRepeatSubmitChecker（与其它验证器的检测器一同，统一为Checker的概念）
* 8.调整 各模板引擎内部接口名称，显得更统一些

#### 1.4.14
* 1.Mapping 的信号类型，由 HTTP 改为 ALL（减少对MethodType的设定）
* 2.所有模板增加 auth 标签支持（** 还需要调试）
* 3.添加 AuthProcessorBase ，支持权限数组的配置方式
* 4.调整主体处理成功后，则立即设为ctx.setHandled(true)；方便after识别404状态
* 5.修复 main action setHandled(true)，after action 不执行的问题
