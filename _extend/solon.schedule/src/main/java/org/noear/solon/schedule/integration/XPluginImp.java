    ...
    public void start(SolonApp app) {
        ...
        Aop.context().beanBuilderAdd(Scheduled.class, (clz, bw, anno) -> {
            if (Runnable.class.isAssignableFrom(clz)) {
                if (anno.fixedRate() > 0) {
                    JobManager.add(anno.name(), anno.fixedRate(), bw.raw());
                } else {
                    JobManager.add(anno.name(), anno.cron(), anno.zone(), bw.raw());
                }
            }
        });
        ...
        Aop.context().beanExtractorAdd(Scheduled.class, (bw, method, anno) -> {
            ...
            if (anno.fixedRate() > 0) {
                JobManager.add(anno.name(), anno.fixedRate(), runnable);
            } else {
                JobManager.add(anno.name(), anno.cron(), anno.zone(), runnable);
            }
        });
