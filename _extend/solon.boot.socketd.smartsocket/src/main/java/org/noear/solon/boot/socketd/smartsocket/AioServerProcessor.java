...
class AioServerProcessor implements MessageProcessor<Message> {
    ...
    public void process(AioSession session, Message message) {
        try {
            ...
            Solon.global().listener().onMessage(session1, message);
        } catch (Throwable ex) {
            EventBus.push(ex);
        }
    }
    ...
    public void stateEvent(AioSession session, StateMachineEnum state, Throwable throwable) {
        ...
        switch (state) {
            case NEW_SESSION:
                Solon.global().listener().onOpen(AioSocketSession.get(session));
                break;

            case SESSION_CLOSED:
                Solon.global().listener().onClose(AioSocketSession.get(session));
            ...
            case OUTPUT_EXCEPTION:
                Solon.global().listener().onError(AioSocketSession.get(session), throwable);
                break;
        }
